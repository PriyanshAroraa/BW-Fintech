...
AccountManager       accountManager(&Indicators);
AdvanceIndicator     AdvIndicators(&Indicators);
MarketCondition      marketcondition(&CSAnalysis, &Indicators, &AdvIndicators, &accountManager);
Probability          probablity(&CSAnalysis, &Indicators, &AdvIndicators, &marketcondition);

TelegramBot          telegramBot(&accountManager);
int                  telegram_result;

OpenPosition         openposition(&Indicators, &accountManager, &AdvIndicators, &CSAnalysis, &marketcondition, &probablity, &telegramBot);
...
void OnTick()
  {
   ...
   for(int i = 0; i < ENUM_TIMEFRAMES_ARRAY_SIZE; i++)
   {
      ...
      isNewBar[i] = false;
      isNewBar[i]=IsNewBar(timeFrameENUM);
